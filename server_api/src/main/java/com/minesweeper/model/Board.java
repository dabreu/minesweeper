...
public class Board {
	 ...
	 * the board is finished. If the cell has no adjacent mines, then the uncover is
	...
	private void doUncover(CellPosition cellPosition) {
		...
		if (cell.isCovered()) {
			...
			if (!cell.hasAdjacentMines()) {
				// if the cell has not adjacent mines, then propagate the uncover to its
				// adjacent cells
				getAdjacents(cellPosition).forEach(position -> doUncover(position));
			}
		}
		// the board will be finished when a mine is found or all the cells without mine
		// are uncovered
	...
	/**
	 * Returns the positions corresponding to the adjacent cells (the square) of a
	 * given cell position
	 * 
	 * @param cellPosition
	 * @return
